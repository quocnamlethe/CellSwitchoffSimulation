...
for l = 1:length(testPert)
    ...
    for m = 0:9        % TODO: FIX SIZE
        ...
        for j = 1:drop
            ...
            waitbar(((l - 1)*10*drop + (m - 1)*drop + j)/(5*10*drop),hwait);
...
%                 CsoTest.TestBs(k).TestPlot(l).SirData = SirData;
        ...
        for k = 1:algNum
            ...
            CsoTest.TestBs(k).TestPlot(l).SirData(m+1,1) = 0.1*m;
            CsoTest.TestBs(k).TestPlot(l).SirData(m+1,2) = mean(RawSIR(k,:,1));
            CsoTest.TestBs(k).TestPlot(l).SirData(m+1,3) = mean(RawSIR(k,:,2));
        end
    end
